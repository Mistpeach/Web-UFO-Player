...
(function () {
  ...
  function Session(opts) {
    ...
    this.conn = null;
    this.opened = false;              // 传输层已打开（WebSocket 握手完成）
    // joined：**已经真的在房间里**（服务器回了 created / joined）。
    //   为什么必须和 opened 分开：WebSocket 的 onopen 早于服务器应答，而服务器拒绝加入
    //   （比如 room-full）时只回一条 error、并不关闭连接。若把"握手成功"当成"进了房间"，
    //   界面就会在主控被拒之后照样切进远程页，并且把 room-full 的提示写进一个已经关掉的弹窗里。
    //   广播/环回没有服务器握手，onOpen 即等价于"已在房间里"。
    this.joined = false;
  ...
  Session.prototype.start = function () {
    ...
    if (kind === 'websocket') {
      this.conn = L.Websocket.open({
        // ⚠ 这里只置 opened，**不置 joined** —— 握手成功 != 进了房间。
        //   真正的 joined 由服务器回的 created / joined 决定（见 _onServerMsg）。
        url: this.url,
      ...
      var c = this.conn;
      c.onOpen = function () { self.opened = true; self.joined = true; self._onOpen(); };
      ...
      cc.onOpen = function () {
        self.opened = true;
        self.joined = true;     // 没有服务器可问，频道开好就等同于进了房间
  ...
  // 是否已经真的在房间里（服务器确认过）。界面据此决定"能不能进远程页"。
  Session.prototype.isJoined = function () { return this.joined; };
  ...
  Session.prototype._onServerMsg = function (msg) {
    ...
    switch (m.t) {
      case T.CREATED:
        this.room = m.room;
        this.opened = true;
        this.joined = true;          // 服务器确认建房成功 —— 这才算进了房间
        this._sendHello();
        this._emit();
        break;
      case T.JOINED:
