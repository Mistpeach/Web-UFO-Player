...
(function () {
  ...
  // 播放头用的相位。
  //   判据是「这条通道是否在输出」：
  //     · 没勾选 / 已取消勾选 → 返回 0，扫描线停在起始位置（时间轴冻结）
  //     · 勾选 → 从 startedAt（勾选那一刻）起算，扫描线正常前进
  //   这条语义是用户明确要求的："取消某个通道输出时，波形扫描线应该停在起始位置"。
  ...
  WC.engine = {
    ...
    // 换波形 = 换了一个动作模板，时间轴要跟着重来：
    //   正在播放（已勾选）时换波形，必须从新波形的**开头**播，
    //   而不是接着旧波形扫描线所在的位置继续 —— 否则新波形会从一个莫名其妙的位置开始。
    //   没在播放的通道不受影响（它的 startedAt 本来就是 0，勾选时才归零起算）。
    setWave: function (name, wave) {
      var c = ch[name];
      var changed = (c.wave !== wave);
      c.wave = wave;
      if (changed && wave && c.enabled && running) {
        c.startedAt = nowMs();     // 时间原点挪到此刻 → 相位回到 0，从头播
      }
    },
