...
(function () {
  ...
  function onSessionStatus(info) {
    ...
    if (info.room && info.room !== S.room) {
      S.room = info.room;
      if (S.step === 'room') {
        $('roomOut').innerHTML = '<div class="roomBig" id="roomBig">' + esc(S.room) + '</div>' +
          '<div class="wzHint">把这个房间号发给主控方，让他输入到「连接房间」里。' +
          '主控连上后本页会自动进入控制状态。</div>';
        var cp = document.createElement('button');
        cp.className = 'wzBtn';
        cp.textContent = '📋 复制房间号';
        cp.onclick = function () {
          try { navigator.clipboard.writeText(S.room); toast('房间号已复制'); }
          catch (e) { toast('复制失败，请手动记录：' + S.room, 'warn'); }
        };
        $('roomOut').appendChild(cp);
      }
    }
